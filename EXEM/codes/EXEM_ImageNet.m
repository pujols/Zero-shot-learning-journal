function EXEM_ImageNet(task, opt, direct_test, feature_name, semantic_name)
...
if strcmp(task, 'train')
    acc_val_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    auc_val_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    HM_val_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    fixed_bias_val_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    acc_val_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    auc_val_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    HM_val_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    fixed_bias_val_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    dis_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    ...
    for f = 1 : nr_fold
        ...
        for g = 1: length(opt.gamma)
            % SVR kernel
            Ker_base = [(1 : length(unique(Ybase)))', exp(-opt.gamma(g) * pdist_bb .^ 2)];
            Ker_hold = [(1 : length(unique(Yhold)))', exp(-opt.gamma(g) * pdist_hb .^ 2)];
            Ker_val = [(1 : length(unique(Yval)))', exp(-opt.gamma(g) * pdist_vb .^ 2)];
            ...
            for d = 1 : length(opt.pca_d)
                ...
                for c = 1 : length(opt.C)
                    for n = 1 : length(opt.nu)
                        ...
                        % Get scores
                        dis_eu(c, n, g, d) = test_EXEM_ImageNet1K(Xval(:, 1 : opt.pca_d(d)), Yval, mean_Xval_rec);
                        ...
                        if strcmp(feature_name, 'googlenet') % sample-wise
                            ...
                            acc_val_eu(c, n, g, d) = acc_val_eu(c, n, g, d) + acc2 / nr_fold;
                        elseif strcmp(feature_name, 'resnet') % class-wise         
                            acc_val_eu(c, n, g, d) = acc_val_eu(c, n, g, d) + evaluate_easy_ImageNet1K(score_U_eu(length(Yhold) + 1 : end, :), Yval, 1) / nr_fold;
                        else
                            disp('Wrong features!')
                            return;
                        end
                        ...
                        auc_val_eu(c, n, g, d) = auc_val_eu(c, n, g, d) + auc / nr_fold;
                        HM_val_eu(c, n, g, d) = HM_val_eu(c, n, g, d) + HM / nr_fold;
                        fixed_bias_val_eu(c, n, g, d) = fixed_bias_val_eu(c, n, g, d) + fixed_bias / nr_fold;    
                        ...
                        if strcmp(feature_name, 'googlenet') % sample-wise
                            ...
                            acc_val_seu(c, n, g, d) = acc_val_seu(c, n, g, d) + acc2 / nr_fold;
                        elseif strcmp(feature_name, 'resnet') % class-wise         
                            acc_val_seu(c, n, g, d) = acc_val_seu(c, n, g, d) + evaluate_easy_ImageNet1K(score_U_seu(length(Yhold) + 1 : end, :), Yval, 1) / nr_fold;
                        else
                            disp('Wrong features!')
                            return;
                        end
                        ...
                        auc_val_seu(c, n, g, d) = auc_val_seu(c, n, g, d) + auc / nr_fold;
                        HM_val_seu(c, n, g, d) = HM_val_seu(c, n, g, d) + HM / nr_fold;
                        fixed_bias_val_seu(c, n, g, d) = fixed_bias_val_seu(c, n, g, d) + fixed_bias / nr_fold; 
                    end
                end
            end
        end
    ...
    save(['../EXEM_CV_results/EXEM_classCV_ImageNet_' feature_name semantic_name '_' norm_method '.mat'],...
...
if (strcmp(task, 'test'))
    ...
    if isempty(direct_test)
        ...
        if strcmp(opt.test_type, 'dis')
            [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(-dis_eu);
        elseif isempty(opt.GZSL)
            ...
            if strcmp(opt.test_type, 'eu')
                ...
                fixed_bias = fixed_bias_val_eu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
            elseif strcmp(opt.test_type, 'seu')
                [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(auc_val_seu);
                fixed_bias = fixed_bias_val_seu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
            else
                disp('Wrong test_type!');
                return;
            end
    ...
    Ker_S = [(1 : length(label_R))', exp(-gamma * pdist2_fast(Sig_R, Sig_R) .^ 2)];
    ...
    if strcmp(opt.test_type, 'dis')
        ...
        Sig_Y = [Sig_S; Sig_U];
        Ker = [(1 : length(label_S) + length(label_U))', exp(-gamma * pdist2_fast(Sig_Y, Sig_R) .^ 2)];
    ...
    if strcmp(opt.test_type, 'dis')
        save (['/projects/vision/zero-shot-learning/data/ImageNet' semantic_name '_' feature_name '_EXEM.mat'], 'w2v', 'no_w2v_loc', 'wnids', 'words', '-v7.3');
    else
        save(['../EXEM_ImageNet_classifiers/EXEM' opt.GZSL '_' opt.test_type '_ImageNet_' feature_name semantic_name '_' norm_method '_C' num2str(C) '_nu' num2str(nu)...
            '_gamma' num2str(gamma) '_pca_d' num2str(pca_d) '.mat'], 'mean_Xtr_PCA' , 'V', 'regressors', 'pca_d', 'avg_std_Xtr', 'gamma', 'fixed_bias', '-v7.3');
    end
end
