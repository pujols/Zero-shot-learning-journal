...
%% Input
% task: 'train', 'test'
% test_type: 'acc_orig_eu', 'acc_orig_seu', 'acc_eu', 'acc_seu'
% dataset: 'AWA', 'CUB', 'SUN'
...
%% Data
[Xtr, Ytr, Xte, Yte, attr2, class_order] = data_loader_GZSL(dataset, opt, feature_name, 'not'); % mainly for new EXEM
...
if (strcmp(task, 'train'))
    ...
    val_acc_orig_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    val_acc_orig_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    ...
    val_HM_orig_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    val_HM_orig_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    ...
    val_bias_orig_eu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
    val_bias_orig_seu = zeros(length(opt.C), length(opt.nu), length(opt.gamma), length(opt.pca_d));
            ...
            for d = 1 : length(opt.pca_d)
                ...
                for c = 1 : length(opt.C)
                    for n = 1 : length(opt.nu)
                        ...
                        [score_S_eu, score_U_eu, score_S_seu, score_U_seu, score_S_orig_eu, score_S_orig_seu]...
                        ...
                        [AUSUC_val, ~, ~, HM, fixed_bias] = Compute_AUSUC(score_S_orig_eu, score_U_eu, Ycomb, label_S, label_U, []);
                        val_acc_orig_eu(c, n, g, d) = val_acc_orig_eu(c, n, g, d) + AUSUC_val / nr_fold;
                        val_HM_orig_eu(c, n, g, d) = val_HM_orig_eu(c, n, g, d) + HM / nr_fold;
                        val_bias_orig_eu(c, n, g, d) = val_bias_orig_eu(c, n, g, d) + fixed_bias / nr_fold;
                        
                        [AUSUC_val, ~, ~, HM, fixed_bias] = Compute_AUSUC(score_S_orig_seu, score_U_seu, Ycomb, label_S, label_U, []);
                        val_acc_orig_seu(c, n, g, d) = val_acc_orig_seu(c, n, g, d) + AUSUC_val / nr_fold;
                        val_HM_orig_seu(c, n, g, d) = val_HM_orig_seu(c, n, g, d) + HM / nr_fold;
                        val_bias_orig_seu(c, n, g, d) = val_bias_orig_seu(c, n, g, d) + fixed_bias / nr_fold;
                    end
                end
            end
        end
    ...
    'val_acc_orig_eu', 'val_acc_orig_seu', 'val_acc_eu', 'val_acc_seu', 'val_HM_orig_eu', 'val_HM_orig_seu', 'val_HM_eu', 'val_HM_seu',...
    'val_bias_orig_eu', 'val_bias_orig_seu', 'val_bias_eu', 'val_bias_seu', 'opt');
end
...
if (strcmp(task, 'test'))
    if(isempty(direct_test) || length(direct_test) == 1)
        load(['../EXEM_CV_results/EXEM_GZSL_classCV_' dataset '_split' num2str(opt.ind_split) '_' feature_name '_' norm_method '.mat'],...
            'val_acc_orig_eu', 'val_acc_orig_seu', 'val_acc_eu', 'val_acc_seu', 'val_HM_orig_eu', 'val_HM_orig_seu', 'val_HM_eu', 'val_HM_seu',...
    'val_bias_orig_eu', 'val_bias_orig_seu', 'val_bias_eu', 'val_bias_seu', 'opt');
        if (strcmp(test_type, 'acc_orig_eu'))
            [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(val_acc_orig_eu, direct_test);
            fixed_bias = val_bias_orig_eu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
        elseif (strcmp(test_type, 'acc_orig_seu'))
            [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(val_acc_orig_seu, direct_test);
            fixed_bias = val_bias_orig_seu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
        ...
        elseif (strcmp(test_type, 'HM_orig_eu'))
            [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(val_HM_orig_eu, direct_test);
            fixed_bias = val_bias_orig_eu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
        elseif (strcmp(test_type, 'HM_orig_seu'))
            [loc_C, loc_nu, loc_gamma, loc_pca_d] = find_max(val_HM_orig_seu, direct_test);
            fixed_bias = val_bias_orig_seu(loc_C(1), loc_nu(1), loc_gamma(1), loc_pca_d(1));
    ...
    [acc_orig_eu, ~, ~, HM_orig_eu, ~] = Compute_AUSUC(score_S_orig_eu, score_U_eu, Yte, label_S, label_U, fixed_bias);
    [acc_orig_seu, ~, ~, HM_orig_seu, ~] = Compute_AUSUC(score_S_orig_seu, score_U_seu, Yte, label_S, label_U, fixed_bias);
    
    disp([acc_eu, HM_eu]);
    disp([acc_seu, HM_seu]);
    disp([acc_orig_eu, HM_orig_eu]);
    disp([acc_orig_seu, HM_orig_seu]);
...
%         save(['../EXEM_results/EXEM_GZSL_' dataset '_' test_type '_split' num2str(opt.ind_split) '_' feature_name '_'...
%             norm_method '_C' num2str(C) '_nu' num2str(nu) '_gamma' num2str(gamma) '_pca_d' num2str(pca_d) '.mat']...
%             , 'regressors', 'C', 'nu', 'gamma', 'pca_d', 'acc_orig_eu', 'acc_orig_seu', 'acc_eu', 'acc_seu', 'HM_eu', 'HM_seu', 'HM_orig_eu', 'HM_orig_seu');
    end
end
end
