function EXEM_ImageNet_test_comb(task, opt, direct_test, feature_name, semantic_name, flat_hit)
...
[Sig_R_MDS, Sig_S_MDS, Sig_U_MDS, ~, ~, ~] = attribute_loader_ImageNet(feature_name, 'MDS', task, opt);
[Sig_S_MDS, median_Dist] = get_class_signatures(Sig_S_MDS, norm_method);
Sig_R_MDS = get_class_signatures_ImageNet_U(Sig_R_MDS, norm_method, median_Dist);
...
load(['../EXEM_ImageNet_classifiers/EXEM_comb' opt.GZSL '_' opt.test_type '_ImageNet_' feature_name semantic_name '_' norm_method '_C' num2str(C) '_nu' num2str(nu)...
            '_gamma' num2str(gamma) '_pca_d' num2str(pca_d) '.mat'], 'mean_Xtr_PCA' , 'V', 'regressors', 'avg_std_Xtr', 'fixed_bias', 'comb_weight', 'comb_scale');
    
if ~isempty(opt.GZSL) && ~isempty(direct_test)
    fixed_bias = direct_test(7);
    comb_scale = direct_test(5);
    comb_weight = direct_test(6);
end

%% Build up the model
Ker_S = [(1 : length(labels_S))', comb_weight * exp(-gamma * pdist2_fast(Sig_S, Sig_R) .^ 2) + (1 - comb_weight) * exp(-comb_scale * pdist2_fast(Sig_S_MDS, Sig_R_MDS) .^ 2)];
Ker_U = [(1 : length(labels_U))', comb_weight * exp(-gamma * pdist2_fast(Sig_U, Sig_R) .^ 2) + (1 - comb_weight) * exp(-comb_scale * pdist2_fast(Sig_U_MDS, Sig_R_MDS) .^ 2)];
...
for i = 1 : length(datafrom)
    ...
    if isempty(opt.GZSL)
        ...
        Y_size = length(Yte);
        save(['../EXEM_ImageNet_results/EXEM_comb' split_or_not opt.GZSL '_' opt.test_type '_ImageNet_' task '_' feature_name semantic_name '_' norm_method ...
...
if isempty(opt.GZSL)
    ...
    total_Y_size = 0;
    for i = 1 : length(datafrom)
        load(['../EXEM_ImageNet_results/EXEM_comb' split_or_not opt.GZSL '_' opt.test_type '_ImageNet_' task '_' feature_name semantic_name '_' norm_method ...
            '_from' num2str(datafrom(i)) '_loc' num2str(loc_index(i)) '.mat'], 'pred_correct', 'acc_class', 'Y_size', 'flat_hit');
    ...
    save(['../EXEM_results/EXEM_comb' split_or_not opt.GZSL '_' opt.test_type '_ImageNet_' task '_' feature_name semantic_name '_' norm_method '.mat'],...
    ...
    acc_sample_wise, HM_sample_wise] = Compute_AUSUC_ImageNet_merge(bias, Y_correct_S, Y_correct_U, class_correct_S, class_count_S, class_count_U, loc_S, loc_U, N_S, N_U, fixed_bias);
        ...
        save(['../EXEM_results/EXEM_comb' split_or_not opt.GZSL '_' opt.test_type '_ImageNet_' task '_' feature_name semantic_name '_' norm_method ...
