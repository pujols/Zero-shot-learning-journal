function [Xtr, Ytr, Xte, Yte, attr2, class_order] = data_loader(dataset, opt, feature_name, EXEM_or_not)
...
if(strcmp(dataset, 'AWA'))
    ...
    if (strcmp(EXEM_or_not, 'yes'))
        load (['../../data/EXEM_info/AWA_EXEM_' feature_name '.mat'], 'attr2');
    end
    Y = Y(:);
...
elseif(strcmp(dataset, 'CUB'))
    ...
    if (strcmp(EXEM_or_not, 'yes'))
        load (['../../data/EXEM_info/CUB_EXEM_' feature_name '_split' num2str(opt.ind_split) '.mat'], 'attr2')
    end
    Y = Y(:);
    X(isnan(X)) = 0; X(isinf(X)) = 0;
    X = bsxfun(@rdivide, X, sqrt(sum(X .^ 2, 2)));
...
elseif(strcmp(dataset, 'SUN'))
    ...
    if (strcmp(EXEM_or_not, 'yes'))
        load (['../../data/EXEM_info/SUN_EXEM_' feature_name '_split' num2str(opt.ind_split) '.mat'], 'attr2')
    end
    Y = Y(:);
    X(isnan(X)) = 0; X(isinf(X)) = 0;
    X = bsxfun(@rdivide, X, sqrt(sum(X .^ 2, 2)));
    ...
    if (strcmp(EXEM_or_not, 'yes'))
        load (['../../data/EXEM_info/' dataset '_EXEM_' feature_name '.mat'], 'attr2')
    end
    Y = Y(:);
    X(isnan(X)) = 0; X(isinf(X)) = 0;
    X = bsxfun(@rdivide, X, sqrt(sum(X .^ 2, 2)));
